...
public class UrlUtils {
    ...
    /**
     *  解析单个URL, 将defaults 中的参数合并到address中
     *  合并逻辑：
     *  1、address看做是url，defaults看做是defaultUrl
     *  2、当url中有不存在属性时，从defaultUrl中获取对应的属性，设置到url中
     *
     * @param address
     * @param defaults
     * @return
    ...
    /**
     * 匹配URL引用的服务是否存在？
     * NOTE: 通过判断暴露的服务的接口、group、版本和当前需要引用的接口、group、版本进行比较
     * @param pattern
     * @param value
     * @return
     */
